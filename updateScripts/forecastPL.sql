SEL A.snpsht_dt
,CAST(a.PL AS DECIMAL(14,2)) AS PL
,A.FTE
,A.PL_RATE
,A.R12
FROM
(
  SEL A.snpsht_dt
  ,ZEROIFNULL(SUM(A.ab_cat_cnt_PL)) AS PL
  ,ZEROIFNULL(SUM(A.full_tm_Eqvlnt_pct)) AS FTE
  ,CAST(CAST( PL AS DECIMAL(14,6))/CAST(FTE AS DECIMAL(14,6)) AS DECIMAL (14,6)) AS PL_RATE
  ,SUM(PL_RATE) OVER(ORDER BY A.snpsht_dt DESC ROWS BETWEEN CURRENT ROW AND 11 FOLLOWING) AS R12
  ,CASE WHEN RANK() OVER(
