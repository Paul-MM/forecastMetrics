...
ts_ongHC <- ts(ts_ongHC$Measure, start = c(2007, 7), frequency = 12) 

#__________________________________________________________________________________________________#

# Non-ongoing hc 
ts_nonHC <- read_csv("data/ts_nonHC.csv")
ts_nonHC <- ts(ts_nonHC$Measure, start = c(2007, 7), frequency = 12)

#__________________________________________________________________________________________________#

# Casual hc 
ts_casHC <- read_csv("data/ts_casHC.csv")
ts_casHC <- ts(ts_casHC$Measure, start = c(2007, 7), frequency = 12) 

#__________________________________________________________________________________________________#

# Total hc 
ts_totHC <- read_csv("data/ts_totHC.csv")
ts_totHC <- ts(ts_totHC$Measure, start = c(2007, 7), frequency = 12) 

#__________________________________________________________________________________________________#

# Ongoing fte
ts_ongFTE <- read_csv("data/ts_ongFTE.csv")
ts_ongFTE <- ts(ts_ongFTE$Measure, start = c(2014, 3), frequency = 12) 

#__________________________________________________________________________________________________#

# Non-ongoing fte
ts_nonFTE <- read_csv("data/ts_nonFTE.csv")
ts_nonFTE <- ts(ts_nonFTE$Measure, start = c(2014, 3), frequency = 12) 

#__________________________________________________________________________________________________#

# Casual fte
ts_casFTE <- read_csv("data/ts_casFTE.csv")
ts_casFTE <- ts(ts_casFTE$Measure, start = c(2014, 3), frequency = 12) 

#__________________________________________________________________________________________________#

# Total fte
ts_totFTE <- read_csv("data/ts_totFTE.csv")
ts_totFTE <- ts(ts_totFTE$Measure, start = c(2014, 3), frequency = 12) 
...
ts_age <- ts(ts_age$Measure, start = c(2007, 7), frequency = 12)
...
ts_apsTnr <- ts(ts_apsTnr$Measure, start = c(2007, 7), frequency = 12) 
